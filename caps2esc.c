...
const struct input_event
    ...
    ctrl_up         = {.type = EV_KEY, .code = KEY_LEFTMETA, .value = 0},
    ctrl_repeat     = {.type = EV_KEY, .code = KEY_LEFTMETA, .value = 2},
    ctrl_down       = {.type = EV_KEY, .code = KEY_LEFTMETA, .value = 1},
...
int should_grab(struct udev_device* device, int initial_scan) {
    ...
    if (libevdev_new_from_fd(fd, &dev) < 0) {
        fprintf(
            stderr,
            "Failed to create evdev device from %s (%s)\n",
            devnode,
            strerror(errno)
        );
        close(fd);
